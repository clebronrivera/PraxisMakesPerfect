...
- **[Script — apply-phase-b.mjs]** New script `scripts/apply-phase-b.mjs` applies `complexity_rationale` + `construct_actually_tested` from `content-authoring/phase-B/output/*.csv` to `questions.json`. Four-category logging: applied / skipped (already matching) / mismatched (existing differs — blocked) / missing (UNIQUEID not in bank). Supports `--dry-run` (no writes) and `--force-mismatches` (overwrite old stubs after review). — `scripts/apply-phase-b.mjs`

- **[Content — Phase B fully applied to questions.json]** Ran `apply-phase-b.mjs --force-mismatches`. Result: 1,368 field writes (104 overwrites of legacy generic stubs with substantive Phase B rationales), 916 skipped, 8 missing (legacy `item_XXX` ACA-06 questions with no CSV entry). Final coverage: `complexity_rationale` 1150/1150 (100%); `construct_actually_tested` 1142/1150 (8 ACA-06 legacy questions are a content gap — no Phase B CSV entries exist for them). — `src/data/questions.json`

- **[Feature — Practice feedback: misconception + knowledge gap block]** After a wrong answer in practice, `ExplanationPanel` now reads `distractor_misconception_{letter}` and `distractor_skill_deficit_{letter}` off the question object and renders a compact "Why this was wrong" callout section (rose border, only when fields are non-empty). No new state or props needed — both fields are already on `AnalyzedQuestion` via the spread-through from questions.json. — `src/components/ExplanationPanel.tsx`

- **[Feature — Study plan: dominant misconception surfaced to Claude]** When `studyPlanPreprocessor` flags a skill as `misconception` status (repeated wrong answer selected 2+ times), it now identifies the dominant wrong-answer letter, finds the most-repeated question for that letter, and looks up `distractor_misconception_*` / `distractor_skill_deficit_*` from questions.json. These are stored as `dominantMisconceptionKey` on `StudentSkillState`, resolved to text in `buildPrecomputedClusters`, and passed to Claude via a new prompt rule: "The student likely believed: [misconception]. Knowledge gap: [skillDeficit]." — `src/types/studyPlanTypes.ts`, `src/utils/studyPlanPreprocessor.ts`, `src/services/studyPlanService.ts`

- **[Feature — Admin item analysis: tier + error-type on distractor chips]** Each distractor frequency chip in the Item Analysis tab now shows `L1`/`L2`/`L3` (color-coded: L1 = rose, L2 = amber, L3 = gray) and `· Conceptual/Procedural/Lexical` when the classification data exists in questions.json. Full misconception text available on hover via `title` attribute. The API endpoint now returns a `distractorDetails` map alongside `distractorFreqs`. — `api/admin-item-analysis.ts`, `src/components/ItemAnalysisTab.tsx`

- **[Feature — Study plan: Phase D skill metadata wired into prompt]** `skill-phase-d.json` is now consumed by the study plan pipeline. `buildPrecomputedClusters` in `studyPlanPreprocessor.ts` looks up each skill's Phase D data (lazy-indexed on first use) and adds `nasp_domain_primary`, `skill_prerequisites`, and `prereq_chain_narrative` to the per-skill cluster payload. Two new prompt rules in `buildPromptV2`: (1) use `prereq_chain_narrative` in domain study maps or cluster actions to name prerequisite dependencies and flag when a prerequisite skill is also weak; (2) use `nasp_domain_primary` to frame cluster advice with NASP practice domain context when multiple skills in a cluster share a domain. TypeScript clean. — `src/utils/studyPlanPreprocessor.ts`, `src/services/studyPlanService.ts`, `src/types/studyPlanTypes.ts`

- **[Content — Phase D applied to skill-phase-d.json]** Ran `apply-phase-d.mjs` after authoring all 5 batches via Coworker. Result: 135 field writes, 0 mismatched, 0 missing. All 45 skills now have `nasp_domain_primary`, `skill_prerequisites`, and `prereq_chain_narrative` in `src/data/skill-phase-d.json`. All 10 NASP domains represented. Script fixed to handle `skill_prerequisites` as array (Coworker output format) in addition to `\n`-delimited string. — `src/data/skill-phase-d.json`, `scripts/apply-phase-d.mjs`

- **[Infrastructure — Phase D standards alignment pipeline]** Built complete Phase D tooling for skill-level standards alignment (45 skills, not per-question): `content-authoring/phase-D/pipeline/extract_phase_d_batch.py` (10-skill batches with sample question stems + Phase B sub-constructs for agent context — 5 batches cover all 45 skills, all can run in parallel), `content-authoring/phase-D/PHASE-D-WORKFLOW.md` (NASP domain reference, field definitions with examples, copyable Coworker prompt), `scripts/apply-phase-d.mjs` (writes/updates `src/data/skill-phase-d.json`; validates `nasp_domain_primary` against NASP-1–10; 4-category logging; creates file on first run). Phase D fields: `nasp_domain_primary`, `skill_prerequisites`, `prereq_chain_narrative`. — `content-authoring/phase-D/`, `scripts/apply-phase-d.mjs`

- **[Infrastructure — Phase C error pattern synthesis pipeline]** Built complete Phase C tooling: `content-authoring/phase-C/pipeline/extract_phase_c_batch.py` (extracts 10-question batches with Phase A distractor classifications embedded, enabling cross-distractor synthesis), `content-authoring/phase-C/PHASE-C-WORKFLOW.md` (field definitions, worked examples, anti-collapse constraints, copyable Coworker prompt), `scripts/apply-phase-c.mjs` (4-category apply script with `error_cluster_tag` validation against TAG_GLOSSARY.md, `--dry-run`, `--force-mismatches`). Phase C fields: `dominant_error_pattern`, `error_cluster_tag`, `instructional_red_flags`. Content authoring not yet started — infrastructure ready for first batch. — `content-authoring/phase-C/`, `scripts/apply-phase-c.mjs`

- **[Fix — ExplanationPanel: complexity_rationale field mismatch]** Pre-existing bug: `ExplanationPanel` checked `(question as any).complexityRationale` (camelCase) but `questions.json` stores the field as `complexity_rationale` (snake_case), and `analyzeQuestion()` never remapped it. The "Complexity" section had never rendered data despite Phase B being fully applied. Fixed by adding `complexityRationale: (q as any).complexity_rationale` to the `analyzeQuestion()` return object and adding `complexityRationale?: string` to `AnalyzedQuestion`. All 1,150 questions now surface their complexity rationale. — `src/brain/question-analyzer.ts`

- **[Feature — ExplanationPanel: "What this tests" section]** Added a new section to the explanation panel that renders `construct_actually_tested` from questions.json. Shows the specific sub-construct the question measures (e.g., "Model-goal discrimination in Caplan consultation"). Renders for all 1,142 questions that have this field populated from Phase B. — `src/components/ExplanationPanel.tsx`

---

### Content Authoring — Phase A Complete + Phase B QC + Folder Reorganization

- **[Content — Phase A distractor classification applied to questions.json]** Applied all 44 Phase A skill CSVs (distractor_tier, distractor_error_type, distractor_misconception, distractor_skill_deficit) to `src/data/questions.json`. 3,587 distractor slots written at 98.7% coverage. 15 slots remain blank across 8 skills — accepted as-is (questions with unusual answer structures or E/F placeholder options). Phase A is complete and archived. — `src/data/questions.json`, `content-authoring/phase-A/output/`

- **[Content — Phase B QC: 16 clean skills applied, 29 collapsed blocked]** Scanned all 45 Phase B CSVs (construct_actually_tested, complexity_rationale) for template collapse — a failure mode where LLM context fatigue causes 1–5 unique strings to be reused across all rows in a skill file. Threshold: ≥80% unique values = clean. Result: 16 clean skills (100% variety) applied → 458 rows written to `questions.json`; 29 collapsed skills (3–43% variety) blocked from apply to prevent contaminating the question bank. — `src/data/questions.json`, `content-authoring/phase-B/output/`

- **[Content — Phase B regen workflow + batch extraction script]** Created complete regeneration workflow for 29 collapsed Phase B skills: `content-authoring/phase-B/PHASE-B-REGEN-WORKFLOW.md` (definition of construct_actually_tested, 5 clean examples, 3 non-examples, 8 agent constraints, copyable Claude.ai Coworker prompt template, parallel wave strategy). Created `content-authoring/phase-B/pipeline/extract_phase_b_batch.py` to generate formatted 10-question blocks for pasting into Coworker agents. 692 questions × 29 skills × ~80 agent batches needed. — `content-authoring/phase-B/PHASE-B-REGEN-WORKFLOW.md`, `content-authoring/phase-B/pipeline/extract_phase_b_batch.py`

- **[Chore — content-authoring folder reorganized by phase]** Restructured `content-authoring/` from a flat layout into phase-labeled subfolders: `phase-A/` (archived — GPT-4o pipeline, SQLite DB, 47 CSVs), `phase-B/` (active — workflow doc, 45 CSVs, batch script), `phase-C/` and `phase-D/` (empty placeholders). Removed stale `extract_for_agent.py` (hardcoded dead paths from a prior session). Updated path references in STATUS.md, PHASE-B-REGEN-WORKFLOW.md, and the batch script. — `content-authoring/`

- **[Docs — Distractor Classification Handoff]** Added `docs/DISTRACTOR_CLASSIFICATION_HANDOFF.md` — standalone agent handoff for Phase A distractor classification work, with full field definitions, tier/error-type rules, 3 fully worked examples, output CSV format, quality standards, and batching strategy. Separate from `docs/CONTENT_AUTHORING_HANDOFF.md` (which covers all four phases). — `docs/DISTRACTOR_CLASSIFICATION_HANDOFF.md`

**Lessons learned — template collapse:** When LLM agents classify long batches of questions in one session, context fatigue causes them to converge on a single construct string that they reuse across all remaining questions. Detection: measure unique_constructs / total_rows per skill file. Prevention: 10 questions per agent session maximum; uniqueness check built into Coworker prompt constraints. This is now documented in the Phase B regen workflow.
