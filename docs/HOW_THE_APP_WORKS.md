...
- **`questionObjectiveMap.json`** tags all 1,200 questions with 1–2 objectives (the 50 newest C4 expansion items carry provisional `method:"manual"`, `verified:false` tags pending the next seeder re-derive). The **human-review queue is cleared**: all **210** genuinely-ambiguous items (139 fallback + 71 multi-tag) have been SME-verified (`method: "manual"`, `verified: true`); the remaining ~940 are high-confidence machine matches (`method: "seeded"`, left `verified: false` because nothing needed disambiguation). Tags live separately from `questions.json`; the seeder preserves manual edits (`--preserve-manual`) and refuses to clobber them otherwise.
...
The landing intentionally does **not** show question-bank size, calibration/IRT claims, or fixed exam counts (for example, "4 domains" / "45 skills" / "1,200 items"). It leads with the adaptive baseline and granular **micro-skill** diagnosis, frames time saved qualitatively (no fabricated hours), and credits an educator with "almost two decades" of classroom/MTSS experience.
...
| Exam domains | 4 |
| Tracked skills | 45 |
| Question bank size | 1,200 questions |
| Wrong-answer options with distractor classification | 3,587 (98.7% of slots) |
| Questions with complexity rationale | 1,200 (100%) |
