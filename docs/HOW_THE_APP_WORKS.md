...
**No psychometric/marketing claims that the product can't back up.** The item bank is expert-authored, **not** IRT-calibrated — the landing and the `?boot=1` boot terminal must never claim "IRT-calibrated", "2PL IRT model", or "calibrated items". Honest proof points only (adaptive baseline · micro-skill diagnosis · plan from your data). The counts are true and fully auditable (1,150 items map cleanly to 45 skills / 4 domains with 0 orphans — verifiable in the admin **Question Bank** tab), but as of 2026-06-02 the **raw item count is intentionally kept out of user-facing landing / tutorial / boot copy for tone**: the welcome slide reads "Practice questions mapped to all 45 skills" and the boot terminal no longer prints "1,150 items." Skill (45) and domain (4) counts may still appear.
