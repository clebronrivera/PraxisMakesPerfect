...
Before sign-in, the public entry page is a multi-section **PASS marketing landing** on a dark twilight (violet/fuchsia) background. PASS branding sits in a sticky top bar with anchor links (How it works · Micro-skills · Our method) and a "Take your baseline" CTA. The page is sequenced **simple-explanation-before-complex-proof**. The sections, in order, are: a **hero** (headline **"Find the exact skills holding you back."**, two benefit-first sentences, and a simple **3-step schematic** — Answer an adaptive baseline → Get your micro-skill map → Study only what moves your score — with a mastered/developing/critical-gap color legend); **How it works** (the four-step loop **Baseline → Diagnose → Target → Monitor**); **"Watch your diagnosis build"** (the animated **PASS Diagnostic Engine** — a neural skill-map that runs the diagnosis on a sample learner as you scroll, then lets you tap any node for a drill-down); **Micro-skill precision** (a Domain → Skill → Micro-skills drill-down); **The method** (**"Grounded in performance science, not guesswork"** — the founder's diagnostic practice, a pull-quote, the Baseline → Diagnose → Target → Monitor → Adjust loop, and role chips); **Why it's faster** (the exam blueprint shown as a grid — most cells already mastered/skipped, a few lit as your targets); **Your plan**; a **founder's note** ("Why I built PASS" by Carlos Lebron Rivera); a closing CTA; and a footer.

The landing intentionally does **not** show question-bank size, calibration/IRT claims, or fixed exam counts (for example, "4 domains" / "45 skills" / exact question counts). It leads with the user benefit and granular **micro-skill** diagnosis, frames time saved qualitatively (no fabricated hours), and sells the founder's **diagnostic experience** — "an educator with 15+ years of diagnostic experience" (analyzing performance data, isolating root causes, matching interventions, monitoring growth) — rather than an MTSS résumé line. The diagnostic engine's numbers (e.g. 68% baseline, 14 micro-skills targeted, 3 critical gaps) are an illustrative **sample learner's** results, not platform statistics.
