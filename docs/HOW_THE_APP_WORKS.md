...
Before sign-in, the public entry page is a multi-section **PASS marketing landing** on a dark twilight (violet/fuchsia) background. PASS branding sits in a sticky top bar with anchor links (How it works · Micro-skills · Our method) and a "Take your baseline" CTA. The page is sequenced **simple-explanation-before-complex-proof**. The sections, in order, are: a **hero** (headline **"Find the exact skills holding you back."**, two benefit-first sentences, and a **micro-skill map** — the four domains, each shown as a row of small tiles colored mastered/developing/critical-gap, with the critical tiles glowing and a color legend below; an illustrative example, not real data); **How it works** (the four-step loop **Baseline → Diagnose → Target → Monitor**); **"Watch your diagnosis build"** (the animated **PASS Diagnostic Engine** — a neural skill-map that runs the diagnosis on a sample learner as you scroll, then lets you tap any node for a drill-down); **Micro-skill precision** (a Domain → Skill → Micro-skills drill-down); **The method** (**"Grounded in performance science, not guesswork"** — the founder's diagnostic practice, a pull-quote, the Baseline → Diagnose → Target → Monitor → Adjust loop, and role chips); **Why it's faster** (the exam blueprint shown as a grid — most cells already mastered/skipped, a few lit as your targets); **Your plan**; a closing CTA; and a footer.
