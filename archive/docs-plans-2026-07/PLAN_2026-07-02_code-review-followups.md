...
| [8] | Verify via `netlify dev` (port 8888): tutor chat works then 429s past limit; study plan happy path; one admin endpoint per changed header path. Merge ⚠️ **PRODUCTION DEPLOY** (Carlos confirm). | 🔄 verified two ways: (1) `tests/apiEndpointGuards.test.ts` — 17 handler-level tests (401 guards, UUID 400, CORS reflect/deny incl. lookalike domain, preflight, dormant Stripe); (2) **deploy-preview-47 full-stack**: evil origin → 401 with NO ACAO, preview/prod origins → ACAO reflected, tutor-chat shared-guard 401, bad UUID → 400, preflight 204. CI `check` green 1m9s. **`netlify dev` cannot exercise worktree code** (CLI resolves repo root via parent's `.git` dir — see memory note). Live 429 needs a real JWT; window math unit-tested. ✅ **MERGED `1793274`** (Carlos "merge and continue", 2026-07-02); **prod verified** ~50s post-deploy: shared-guard 401 live, evil origin → zero ACAO headers, bad UUID → 400 |

## PHASE 3 — Doc rot cleanup  ·  *cheap, no code*  ·  **PR C**

| # | Task | Status |
|---|---|---|
| [9] | Archive stale root docs per repo convention (`archive/docs-cleanup-2026-07/`): `CODEBASE_OVERVIEW.md`, `REWRITE_DEVELOPMENT_GUIDE.md`, `ASSESSMENT_DATA_FLOW_ANALYSIS.md`, `HANDOFF_2026-04-27.md` — or stamp a dated deprecation banner if Carlos prefers keeping them at root (check `docs/DOCS_SYSTEM.md` first; **fix that doc too if it lists any archived file as canonical**). | ✅ hybrid per DOCS_SYSTEM + the "canonical root docs stay at root" convention: the 3 canonical docs got dated ⚠️ STALE banners (+ staleness notes on their DOCS_SYSTEM entries); the point-in-time `HANDOFF_2026-04-27.md` archived to `archive/docs-cleanup-2026-07/`. Bonus README fixes: dead root links → `docs/` paths; superseded "PMP in maintenance / PASS elsewhere" status note replaced (this IS the active PASS repo, auto-deploys from main) |
| [10] | Update `CHANGELOG.md` through PR #44 (Phase 2 retake/A4, C4 bank 1,200+, staged study guide, 0027/0028, tutor model fix). Durable copy — no exact counts that go stale. | ✅ 2026-07-02 section (Phases 1–3 remediation, PRs #46/#47) + 2026-06-12→16 backfill (PRs #37/#38/#41/#42/#44/#45); durable copy |

## PHASE 4 — Test seams for the refactor  ·  **PR D**  ·  *gate for Phase 5*

| # | Task | Status |
|---|---|---|
| [11] | Add React Testing Library integration tests (first `*.test.tsx` files) for the 3 highest-risk flows: **adaptive-diagnostic assessment flow** (start → answer → complete → unlock), **PracticeSession** (answer/streak/retirement + redemption wiring), **OnboardingFlow** (validation + submit). Mock Supabase at the hook boundary. Target: behavior pinning, not coverage %. | ✅ 21 tests in 3 suites: `adaptiveDiagnosticFlow` (7 — mount resume-pointer, correct/wrong logging, follow-up queueing + is_followup, 3-per-skill cap, onComplete payload + session cleanup, pause/exit, resume-at-index), `practiceSessionFlow` (7 — stats/streak, persistence seam, onWrongAnswer/onAnswerSubmitted, hint quarantine incl. no-score + single-fire onHintRedemption, blacklist pool exclusion, localStorage retirement), `onboardingFlow` (7 — step gating + named-field hints, role-dependent step count, grad 4-step submit payload, retake reveal, save-error retry, skip). Supabase mocked at useAuth/glossaryService boundary; App-provided callbacks asserted as the seam. Note: the post-completion **unlock** gate lives in App.tsx (~:456) and is pinned via the onComplete contract — direct unlock assertions land with Phase 5's `useAssessmentOrchestration`. Infra: `@testing-library/react`+`user-event`+`jest-dom`+`jsdom` devDeps; per-file `@vitest-environment jsdom`; `tests/setup.ts` (jest-dom matchers + RTL cleanup) |
| [12] | Wire into `npm test` / CI (`npm run check`) — confirm CI still < ~3 min. | ✅ vitest include broadened to `tests/**/*.test.{ts,tsx}` → runs in `npm test` → already inside `npm run check` + CI. Full suite 35 files / 319 tests in ~2.6s local; gate (types+colors+lint+test+build) green |
...
| # | Task | Status |
|---|---|---|
| [13] | **E1 — `useAssessmentOrchestration`**: compose `useAssessmentFlow` (879 ln) + the 4 retake `useState`s + adaptive wiring behind one hook with a clean `{ startScreener, startAdaptive, startRetake, resume }` API. ~400 lines out of App.tsx. | ✅ `src/hooks/useAssessmentOrchestration.ts` (320 ln): spreads full `useAssessmentFlow` + retake lifecycle (state, A4/D8 unlock-gate memo, start/complete/exit/dismiss), in-progress derivations (hasAssessmentInProgress/hasPracticeInProgress/hasOrphanedAdaptive), `resumeFromLastSession`/`discardInProgressSession` (the ~60-ln home Resume onClick moved verbatim, incl. orphaned-adaptive Supabase reconstruction + diagnostic→screener migration), `diagnosticSummary` memo. Verbatim; gate green |
| [14] | **E2 — `useRedemptionFlow`** (redemption + retake pass-through state, ~120 ln) and **`useSocialHub`** (leaderboard open/mode + users-online sim, ~80 ln — keep `getHourRange` + drift effect intact per CLAUDE.md). | ✅ `useRedemptionFlow.ts` (76 ln, composes `useRedemptionRounds` + round-question state + handleStartRedemption/completeRound) and `useSocialHub.ts` (83 ln, getHourRange 24-entry table + drift effect byte-identical per CLAUDE.md + `useLeaderboard` passthrough). Verbatim; gate green |
| [15] | **E3 — split `useProgressTracking`** (993 ln) → `useUserProfile` / `useProgressLogging` / `useScoreRecalculation`, with `useProgressTracking` kept as a thin composing facade so the 8+ call sites don't all change at once. **Do not touch the `cea4af9` question_id dedupe logic — locked per memory.** | ✅ 993 ln → 71-ln facade + `useUserProfile` (545: profile state/persistence, debounced save, onboarding, migration, lastSession, reset — owns the single profile source + exposes profileRef/setProfileState/saveProfile seams) + `useProgressLogging` (377: responses reads/writes) + `useScoreRecalculation` (159: updateSkillProgress incl. **untouched cea4af9 dedupe** + recalculateGlobalScores). Facade re-exports `UserProfile`/`ResponseLog` so all call sites resolve unchanged. Adversarial verification workflow run (6 lenses → per-finding refutation). Gate green |
| [16] | Gate + manual smoke via `netlify dev` of assessment / practice / redemption / leaderboard. Merge ⚠️ **PRODUCTION DEPLOY** (Carlos confirm). Target: App.tsx ≤ ~1,400 lines. | 🔄 gate green (types+colors+lint+319 tests incl. 21 Phase-4 pins + build); App.tsx 2,038 → 1,832 (short of the ≤1,400 target — remaining bulk is the home-screen JSX render tree, not logic; a JSX-extraction pass would be a separate E4). `netlify dev` smoke can't run from a worktree (parent-checkout gotcha) — verify on deploy preview. **Awaiting Carlos merge confirm** |

## PHASE 6 — Question-bank load strategy  ·  **PR F**

| # | Task | Status |
|---|---|---|
| [17] | **Decide approach (Carlos):** (a) defer the 6.3MB fetch until an assessment/practice mode is entered (smallest change, recommended), or (b) split `questions.json` into per-domain chunks loaded on demand (bigger win, more moving parts). Extend the existing lazy pattern from `studyPlanPreprocessor.ts:367` either way. | ✅ **(a) defer-fetch** chosen by Carlos 2026-07-02 |
| [18] | Implement + verify: network tab shows no bank download on login/dashboard; assessment start latency acceptable on throttled connection; all 1,200+ questions still resolve (existing `questionsJsonSchema` test + count assertion). | ✅ App.tsx: eager mount-fetch replaced by idempotent ref-guarded `ensureQuestionBank()` (returns the analyzed bank; retries on failure); removed the bank from the top loading gate so login/onboarding/dashboard render instantly. Threaded `ensureQuestionBank` into usePracticeFlow / useAssessmentFlow / useAssessmentOrchestration / useRedemptionFlow — every build/entry (`startScreener`/`startFullAssessment`/`startAdaptiveDiagnostic` incl. resume+Supabase-reconstruction / `startRetakeAssessment` / `handleViewReport` / `handleStartRedemption` / `startPractice`/`startSkillPractice`) awaits it and uses the resolved array (no empty-bank leak). Mode-prefetch effect + `questionBankReady` render-gates (loading/error+retry panel) on practice/results/learning-path absorb the async gap. **Browser-verified** on the login screen: `performance` shows **0** `questions.json` fetches (only the small metadata JSONs load), app renders, and the deferred URL resolves to 6.28MB / 1,241 questions when triggered. Gate green (types+colors+lint+319 tests+build). Auth'd dashboard→assessment trigger guaranteed by code (sole fetch site) — full-path smoke needs a deploy preview / real creds. |
...
2. **[9] Stale root docs** — ✅ resolved without needing a call: DOCS_SYSTEM lists 3 of the 4 as canonical and the standing convention is "canonical root docs stay at root," so those got stale banners in place; only the handoff (point-in-time by nature, not in DOCS_SYSTEM) was archived.
