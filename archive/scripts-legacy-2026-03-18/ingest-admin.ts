...
function getAdminApp(): App {
  ...
  const credentialPath = path.resolve(process.cwd(), 'serviceAccountKey.json');
  return initializeApp({
    credential: cert(JSON.parse(readFileSync(credentialPath, 'utf8')))
  });
}

const db = getFirestore(getAdminApp());async function ingestData(questionsPath: string, skillsPath: string) {
  ...
  for (const row of skillsData) {
    ...
    batch.set(skillsRef.doc(skill.id), skill);
    operationCount++;
  ...
  for (const row of questionsData) {
    ...
    batch.set(questionsRef.doc(question.id), question);
    operationCount++;
