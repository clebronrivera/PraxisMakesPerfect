...
ANTI-TEMPLATE RULES (strictly enforced — these are the most common failure modes):
  - FORBIDDEN: "confusing the task demand in [question stem]..."
  - FORBIDDEN: "missing the key principle described in the explanation"
  - FORBIDDEN: "Student assumed '[answer text]' was the best interpretation..."
  - FORBIDDEN: "confusing the scenario demand in [question text]..."
  - FORBIDDEN: "Distinction between [correct answer] and distractor approach in [context]"
  - FORBIDDEN: "matched the clinical need in this case, confusing the scenario demand..."
  - FORBIDDEN: Any distractor_skill_deficit that contains words copied from the answer options or question stem
  - FORBIDDEN: "Student mistakenly selects an option related to..." — old boilerplate

  distractor_misconception must answer: "What did this student BELIEVE was true?"
  distractor_skill_deficit must answer: "What specific knowledge concept is missing?" (name the concept, not describe the question)

  Self-check before writing each row: Could I paste this misconception onto a different wrong answer
  in the same question and have it still make sense? If yes, it is too generic — rewrite it.
...
6. Save the output to:
   ...
   IMPORTANT: Cursor only works within the content-authoring/ folder.
   It reads extracted .txt files and writes output CSVs. It does NOT touch questions.json.

7. Update STATUS.md — change ⬜ Pending → 🔄 In Progress for that skill/phase.
   Only mark ✅ Done after Claude Code has applied the output to questions.json.

8. (Claude Code step — separate from Cursor) Apply the output to src/data/questions.json.
   Run this in a Claude Code session (not Cursor):
   "Apply content-authoring/output/phase-A/[SKILL-ID]-phase-A.csv
    to questions.json — update the matching distractor fields for each UNIQUEID."

   After a successful apply, update STATUS.md: 🔄 In Progress → ✅ Done.
