...
`.trim();
  ...
  return [
    ...
    '- For skills with dominantMisconception present: reference it in the cluster whyItMatters or domainStudyMaps commonTraps using the phrasing "The student likely believed: [dominantMisconception]." followed by "Knowledge gap: [dominantSkillDeficit]." if dominantSkillDeficit is also present. Treat this as a probable belief, not a certainty.',
    '- For clusters with dominantErrorClusters present (3+ missed questions sharing the same error_cluster_tag): call out the pattern explicitly in whyItMatters using the tag name. Example: "Across your Legal & Ethics questions, your most consistent error pattern is scope-overgeneralization (6 missed questions). This suggests you are confusing the boundaries of specific legal protections." Use the tag count to convey severity.',
    '- For skills with prereq_chain_narrative present: use this in domainStudyMaps or cluster keyActions to name the prerequisite knowledge this skill depends on. When a prerequisite skill is also weak, flag the dependency explicitly.',
    '- nasp_domain_primary identifies the NASP practice domain for each skill (e.g. NASP-2 = Consultation, NASP-10 = Legal/Ethical). When multiple skills in a cluster share the same nasp_domain_primary, you may frame the cluster advice using that domain label to give the student a professional context anchor.',
