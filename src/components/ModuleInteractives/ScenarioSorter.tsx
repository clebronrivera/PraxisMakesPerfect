...
export default function ScenarioSorter({
  ...
  const getCategoryColor = (category: string) => {
    if (category.includes('GREEN') || category.includes('YES') || category.includes('ENDORSED')) {
      return isA ? 'bg-[color:#059669]/10 border-[color:#059669]/30' : 'bg-emerald-50 border-emerald-200';
    }
    if (category.includes('RED') || category.includes('NO') || category.includes('NOT')) {
      return isA ? 'bg-[color:#e11d48]/10 border-[color:#e11d48]/30' : 'bg-rose-50 border-rose-200';
    }
  ...
  const getCategoryBadgeColor = (category: string) => {
    if (category.includes('GREEN') || category.includes('YES') || category.includes('ENDORSED')) {
      return isA ? 'bg-[color:#059669]/20 text-[color:#059669]' : 'bg-emerald-100 text-emerald-700';
    }
    if (category.includes('RED') || category.includes('NO') || category.includes('NOT')) {
      return isA ? 'bg-[color:#e11d48]/20 text-[color:#e11d48]' : 'bg-rose-100 text-rose-700';
    }
    return isA ? 'bg-[color:#0284c7]/20 text-[color:#0284c7]' : 'bg-blue-100 text-blue-700';
  };
  ...
  const scenarioText = isA ? 'text-sm text-slate-700' : 'text-sm text-slate-700';
  const emptyPlaceholder = isA ? 'text-xs text-slate-500 italic' : 'text-xs text-slate-400 italic';
  const placedNeutralCls = isA ? 'bg-slate-50 border-slate-200 text-slate-700' : 'bg-white border-slate-200 text-slate-700';
  const placedCorrectCls = isA
    ? 'bg-[color:#059669]/10 border-[color:#059669]/40 text-slate-900'
    : 'bg-emerald-50 border-emerald-300 text-emerald-800';
  const placedWrongCls = isA
    ? 'bg-[color:#e11d48]/10 border-[color:#e11d48]/40 text-slate-900'
    : 'bg-rose-50 border-rose-300 text-rose-800';
  const checkIcon = isA ? 'w-4 h-4 text-[color:#059669] shrink-0 mt-0.5' : 'w-4 h-4 text-emerald-600 shrink-0 mt-0.5';
  const xIcon = isA ? 'w-4 h-4 text-[color:#e11d48] shrink-0 mt-0.5' : 'w-4 h-4 text-rose-500 shrink-0 mt-0.5';
  const correctHintText = isA ? 'text-xs mt-1.5 ml-6 text-[color:#e11d48]' : 'text-xs text-rose-600 mt-1.5 ml-6';
  const submitBtnCls = isA
    ? 'editorial-button-primary w-full text-sm font-semibold'
    : 'w-full py-2 rounded-lg bg-cyan-600 hover:bg-cyan-700 text-slate-900 text-sm font-semibold transition-colors border border-transparent';
