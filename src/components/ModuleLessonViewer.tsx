...
function SectionRenderer({
  ...
  const bodyBase = isA ? 'text-slate-700' : 'text-slate-700';
  ...
  const compRightShell = isA
    ? 'bg-[color:#d97706]/10 rounded-xl border border-[color:#d97706]/30 overflow-hidden'
    : 'bg-indigo-50 rounded-xl border border-indigo-200 overflow-hidden';
    ...
    case 'interactive': {
      const completionBadge = completed?.completed ? (
        <div
          className={
            isA
              ? 'flex items-center gap-1.5 rounded-lg bg-[color:#059669]/10 border border-[color:#059669]/30 px-3 py-1.5 mt-3'
              : 'flex items-center gap-1.5 rounded-lg bg-emerald-50 border border-emerald-200 px-3 py-1.5 mt-3'
          }
        >
          <CheckCircle
            className={isA ? 'w-3.5 h-3.5 text-[color:#059669]' : 'w-3.5 h-3.5 text-emerald-600'}
          />
          <span className={isA ? 'text-xs font-bold text-[color:#059669]' : 'text-xs font-bold text-emerald-700'}>
...
export default function ModuleLessonViewer({
  ...
  const shellCls = isA
    ? 'bg-[#ffffff] backdrop-blur-[14px] rounded-2xl border border-slate-200 overflow-hidden'
  ...
  const moduleIdPill = isA
    ? 'bg-[color:#d97706]/15 text-[color:#d97706] text-xs font-bold px-2 py-1 rounded-md uppercase tracking-wider border border-[color:#d97706]/30'
    : 'bg-indigo-100 text-indigo-700 text-xs font-bold px-2 py-1 rounded-md uppercase tracking-wider';
  const timeCls = isA
  ...
  const relatedId = isA
    ? 'text-[10px] font-mono text-[color:#d97706] mr-2'
    : 'text-[10px] font-mono text-indigo-700 mr-2';
  const relatedTitle = isA ? 'text-xs text-slate-600' : 'text-xs text-slate-600';
  const viewedBorder = isA ? 'pt-3 border-t border-slate-200' : 'pt-3 border-t border-slate-100';
  const viewedOn = isA
    ? 'bg-[color:#059669]/10 border-[color:#059669]/50 text-[color:#059669] hover:bg-[color:#059669]/15'
    : 'bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100';
  const viewedOff = isA
    ? 'bg-slate-50 border-slate-200 text-slate-600 hover:border-[color:#d97706]/40 hover:text-slate-900 hover:bg-slate-50'
      ...
      <div className={`${contentGap} ${contentPad}`}>
        ...
        <div className={viewedBorder}>
          <button
            ...
            className={`w-full flex items-center justify-center gap-2.5 py-3 rounded-xl border-2 font-bold text-sm transition-all focus-visible:outline focus-visible:outline-2 focus-visible:outline-[color:#d97706] ${
