// Builds the Word (.docx) score report.
//
// This module is deliberately NOT imported statically anywhere. `docx` is ~400 kB
// and this report is only produced when a user clicks "Download Detailed Report",
// but ScoreReport renders on the highest-traffic screen in the product — straight
// after the diagnostic. `scoreReportGenerator.ts` reaches this file through a
// dynamic import so the whole builder ships as its own lazy chunk. Import it
// statically and that 400 kB goes back on the post-diagnostic critical path.
import { Document, Paragraph, TextRun, Table, TableRow, TableCell, ShadingType, WidthType, BorderStyle, AlignmentType, HeadingLevel, VerticalAlign } from 'docx';
