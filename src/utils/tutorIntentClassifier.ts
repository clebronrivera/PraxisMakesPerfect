...
const APP_GUIDE_TRIGGER_PATTERNS = [
  ...
  /\bwhere\s*(do\s*i|can\s*i|should\s*i|is\s*the|'?s\s*the|are\s*the)\b/i,
  /\bwhere\s*(is|are)\b/i,
  /\bwhere\s*do\s*(i|you|we)\b/i,
  /\bexplain\s*the\s*(app|interface|dashboard)\b/i,
  /\bhelp\s*me\s*(get\s*started|understand|find|navigate)\b/i,
  /\bwhat\s*should\s*i\s*do\s*(first|next|now)\b/i,
  /\btutorial\b/i,
  /\bhow\s*(do\s*i|can\s*i)\s*(find|get\s*to|access|open|start)\b/i,
  /\bnavigate\b/i,
  /\bget\s*started\b/i,
  /\bwhat('?s|\s+is)\s*(the\s*)?(app|this)\b/i,
  /\bhow\s*(does|do)\s*(this|the)\s*(work|app|feature)\b/i,
  /\bunlock\b/i,
  /\bwhat\s*percentage\b/i,
  /\bhow\s*many\s*(skills|questions|credits)\b/i,
  /\bwhat\s*(level|percent|score)\b.*\b(need|require|pass|ready)\b/i,
  /\bproficiency\b/i,
  /\bmastery\b/i,
  /\breadiness\b/i,
  /\bredemption\b/i,
